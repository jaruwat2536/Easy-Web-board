...
<body>
		...
		<?
			$p_id = $_GET["id"];
			$qry=mysql_query("select * from post where p_id =$p_id");
			$title_id=mysql_result($qry,0,"title_id");
		?>
        ...
        <div class="col-md-8">
	        <div class="panel panel-info">
			  ...
			  <form name="form" method="POST" action="update_post.php?id=<? echo $p_id; ?>">
		      	<div class="form-group">
				  <label for="usr">Title:</label>
				  <input type="text" class="form-control" id="usr" name="usr" value=<? echo mysql_result($qry,0,"title"); ?> >
				</div>
				<div class="form-group">
				  <label for="comment">Body:</label>
				  <textarea class="form-control" rows="5" id="comment" name="comment"><? echo mysql_result($qry,0,"p_body"); ?></textarea>
				</div>
				<div class="col-md-10"></div>
				<select name="type" id="type">
				<? $qry_pt3=mysql_query("select * from post_type where title_id='$title_id'");?>
					<option value="<? echo mysql_result($qry_pt3,0,"title_id"); ?>" select=select><? echo mysql_result($qry_pt3,0,"type_name"); ?></option>
				<?
					$qry_pt2=mysql_query("select * from post_type where title_id<>'$title_id'");
